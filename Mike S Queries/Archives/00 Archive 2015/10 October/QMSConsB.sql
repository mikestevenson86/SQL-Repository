...
BEGIN
	...
	SELECT @ref = RefNo FROM SalesforceReporting..QMSAuditData_Cons_B WHERE RefNo = @row
	...
	BEGIN
		...
		SELECT @columnname = COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
					WHERE TABLE_NAME = 'QMSAuditData_Cons_B' AND ORDINAL_POSITION = @rn

		SET @sql = 'INSERT INTO NEWQMSConsB (DueDate, ContractNo, AccountNo, Narrative, DayNumber, Value) ' +
		'SELECT ' + CHAR(39) + CONVERT(varchar,@dued) + CHAR(39) + ', [ContractNo], ' +
		'[AccountNo], [Product], DayNumber, ' + @columnname +
		' FROM QMSAuditData_Cons_B WHERE [RefNo] = ' + @ref
...
--DELETE FROM SalesforceReporting..NEWQMSConsB
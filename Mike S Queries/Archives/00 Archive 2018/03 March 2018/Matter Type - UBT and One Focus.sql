...
SELECT
	...
	WHERE
		appraisal = 1
		AND
		cit_sh_advice.dateOfCall >= '2017-03-16'
		AND
		cit_sh_advice.dateOfCall < DATEADD(d , 1, '2018-03-17') -- Incremrement by a day
		AND
		cit_sh_advice.del <> 1
	UNION ALL
	SELECT
...
left outer join Shorthorn..cit_sh_clients cl ON detail.[Shorthorn ID] = cl.clientID
WHERE ISNULL(onefocusURN, '') <> '' or ISNULL(ubtURN, '') <> ''